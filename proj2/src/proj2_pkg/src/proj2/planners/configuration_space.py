...
class BicycleConfigurationSpace(ConfigurationSpace):
    ...
    def distance(self, c1, c2):
        ...
        x_diff = c2[0] - c1[0]
        y_diff = c2[1] - c1[1]
        theta_diff = min(np.abs(c2[2] - c1[2]), 2 * np.pi - np.abs(c2[2] - c1[2]))
        return np.sqrt(x_diff**2 + y_diff**2) + 0.5*theta_diff
    ...
    def sample_config(self, *args):
        ...
        """
        x, y = np.random.uniform(self.low_lims[0], self.high_lims[0]), np.random.uniform(self.low_lims[1], self.high_lims[1])
        theta = np.random.uniform(-np.pi, np.pi)
        phi = np.random.uniform(self.input_low_lims[1], self.input_high_lims[1])
