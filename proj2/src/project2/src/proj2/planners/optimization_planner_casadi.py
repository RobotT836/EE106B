...
def bicycle_robot_model(q, u, L=0.3, dt=0.01):
    ...
    x_dot = u1 * cos(theta)
    y_dot = u1 * sin(theta)
    theta_dot = u1 * tan(phi) / L
    phi_dot = u2
    q_next = vertcat(x + x_dot * dt, y + y_dot * dt, theta + theta_dot * dt, phi + phi_dot * dt)
...
def initial_cond(q_start, q_goal, n):
    ...
    # Your code here.
    x = q_goal[0] - q_start[0]
    y = q_goal[1] - q_start[1]
    theta = np.arctan2(y, x) 
    q0[0] = np.linspace(q_start[0], q_goal[0], n + 1)
    q0[1] = np.linspace(q_start[1], q_goal[1], n + 1)
    q0[2] = theta
    q0[3] = 0 # phi
...
def constraints(q, u, q_lb, q_ub, u_lb, u_ub, obs_list, q_start, q_goal, L=0.3, dt=0.01):
    ...
    constraints.append(q[q.shape[1] + 1] == q_goal) # Constraint on final state.
